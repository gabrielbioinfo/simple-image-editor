...
const EditorTools = (
  ...
  const { drawLineColor, setDrawLineColor, history, futureHistory } = useImageStore(
    (state) => state,
  )
  ...
  const handleRotateLeft = (evt: any) => {
    evt.preventDefault()
    if (rotateLeftAction) rotateLeftAction()
  }

  const handleRotateRight = (evt: any) => {
    evt.preventDefault()
    if (rotateRightAction) rotateRightAction()
  }

  const handleCrop = (evt: any) => {
    evt.preventDefault()
    if (cropAction) cropAction()
  }
  ...
  const handleCanDraw = (evt: any) => {
    evt.preventDefault()
    if (canDrawAction) canDrawAction()
  }
  ...
  const handleZoomIn = (evt: any) => {
    evt.preventDefault()
    if (zoomInAction) zoomInAction()
  }

  const handleZoomOut = (evt: any) => {
    evt.preventDefault()
    if (zoomOutAction) zoomOutAction()
  }
  ...
  return (
    <form className="flex gap-1.5 items-left bg-gray-600 p-3 rounded-lg">
      <div className="flex gap-1.5 items-left">
        ...
        <button
          className="hidden flex items-center p-1 px-2 text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-semibold rounded-lg shadow-md focus:outline-none"
          title="crop"
        ...
        <div className={`flex gap-0.5 items-center`}>
          ...
          <div className="flex items-center">
            <input
              disabled={!canDraw}
              onChange={handleChangeDrawLineColor.bind(this)}
              type="color"
              className="p-1 w-8 block bg-white border cursor-pointer rounded-md disabled:opacity-50 disabled:pointer-events-none bg-neutral-900 border-neutral-700"
              id="hs-color-input"
              defaultValue={drawLineColor}
